...
public class ActionLogger extends Suspendable {
    ...
    public void saveToDB() {
        pool.run(connection -> {
            String sql = "INSERT INTO player_boards " +
                    "(time, id, game_type, score, command, board) " +
                    "VALUES (?,?,?,?,?,?);";
    ...
    public long getLastTime(String id) {
        return pool.select("SELECT MAX(time) AS time FROM player_boards WHERE id = ?;",
                new Object[]{ id },
    ...
    public List<BoardLog> getBoardLogsFor(String id, long time, int count) {
        return pool.select(
                    "SELECT * FROM (SELECT * FROM player_boards WHERE id = ? AND time <= ? ORDER BY time ASC LIMIT ?) AS before" +
                    " UNION " +
                    "SELECT * FROM (SELECT * FROM player_boards WHERE id = ? AND time > ? ORDER BY time ASC LIMIT ?) AS after;",
                new Object[]{
